...
1. 서버는 HTTP 응답 헤더의 `Set-Cookie`를 통해 쿠키 생성을 지시
2. 브라우저는 쿠키를 저장
3. 이후 요청에서 브라우저는 `Cookie` 헤더에 쿠키를 포함시켜 서버로 전송
...
1. 사용자가 처음 접속하면 서버는 유니크한 세션 ID 생성
2. 서버는 세션 ID를 쿠키에 저장하여 클라이언트에 전송
3. 서버는 세션 ID에 해당하는 사용자 데이터를 메모리, 데이터베이스 등에 저장
4. 클라이언트가 재접속하면 쿠키의 세션 ID를 통해 서버에서 해당 사용자 데이터 조회
...
2. **페이로드(Payload)**: 클레임(claim)이라 불리는 엔티티와 추가 데이터 포함
   ```json
   {
     ...
     "name": "baekseungjin",  // custom claim
...
1. 사용자 인증 후 서버가 JWT 생성해 클라이언트에 반환
2. 클라이언트는 모든 요청 시 Authorization 헤더에 JWT 포함
3. 서버는 JWT 검증으로 요청의 인증/권한 확인
4. 토큰 만료 시 재발급 필요
...
- Refresh Token과 Access Token 패턴 이해 필요
  - Access Token: 짧은 유효기간, 리소스 접근 용도
  - Refresh Token: 긴 유효기간, 새 Access Token 발급 용도
- HS256(대칭키) vs RS256(비대칭키) 알고리즘 선택 시 고려사항

**참고로 jwt 구현할 때 ["개발자 유미"](https://www.devyummi.com/page?id=668cfe58d3b43a6241eb6b6c)라는 분께서 잘 정리해주신게 있으니 참고**
